...
export default function RecipesClient() {
  ...
  const [tab, setTab] = useState<'list'|'detail'|'monthly'|'variance'|'simulation'>('list')
  ...
  const simResults = recipes.map(recipe => {
    ...
    ings.forEach(ing => {
      ...
      const newPrice = simItem ? simItem.new_price : oldPrice
    ...
    return {
      ...recipe, ings, oldCost, newCost, sp, oldMargin, newMargin,
      costIncrease, sold, monthlyImpact,
      affected: costIncrease !== 0
    }
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl flex-wrap">
        {(['list','detail','monthly','variance','simulation'] as const).map(t => (
          <button key={t} onClick={() => { setTab(t); if(t==='monthly'||t==='variance') loadMonthData() }}
            className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-colors ${tab===t ? (t==='simulation' ? 'bg-purple-700 text-white shadow-sm' : 'bg-white text-gray-900 shadow-sm') : 'text-gray-500 hover:text-gray-700'}`}>
            {t === 'list' ? 'Recipe List' : t === 'detail' ? 'Recipe Detail' : t === 'monthly' ? 'Monthly Cost' : t === 'variance' ? 'Variance Analysis' : '⚡ Price Simulation'}
          </button>
        ))}
      </div>
      ...
      {tab === 'simulation' && (
        <div className="flex flex-col gap-5">
          ...
          {/* Month selector for monthly impact */}
          <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm flex flex-wrap gap-3 items-end">
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">Year (for monthly impact)</label>
            ...
            {Object.keys(simMealCounts).length > 0 && <span className="text-xs text-green-600 font-medium">✓ {Object.keys(simMealCounts).length} items loaded</span>}
          </div>

          {/* Add items to simulate — search */}
            ...
            {simSearch.length > 1 && (
              <div className="mt-2 border border-gray-200 rounded-lg overflow-hidden max-h-48 overflow-y-auto">
                {masterItems.filter(m =>
                  m.item_name.toLowerCase().includes(simSearch.toLowerCase()) ||
                  String(m.item_code).includes(simSearch)
                ).slice(0,10).map(m => (
                  <div key={m.item_code} onClick={() => addSimItem(m)}
                    ...
                    <span className="text-gray-400 font-mono">{m.correct_price.toFixed(5)} KWD</span>
                    {simItems.find(s=>s.item_code===m.item_code)
                      ? <span className="text-purple-600 text-xs ml-2">✓ Added</span>
                      : <span className="text-blue-600 text-xs ml-2">+ Add</span>}
                  </div>
                ))}
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  { label:'Total Monthly Impact',   value: 'KWD '+(totalMonthlyImpact >= 0 ? '+' : '')+totalMonthlyImpact.toFixed(3),
                    color: totalMonthlyImpact > 0 ? 'text-red-600' : 'text-green-600' },
